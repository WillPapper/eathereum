...
impl RedisConsumer {
                ...
                for stream_key in stream_reply.keys {
                    if stream_key.key == self.config.stream_key {
                        for stream_id in stream_key.ids {
                            match StreamMessage::from_redis_stream(stream_id.id.clone(), &stream_id.map) {
                                Ok(msg) => {
    ...
    pub async fn read_pending_messages(&mut self) -> Result<Vec<StreamMessage>> {
        let result = {
            ...
            let result: redis::RedisResult<HashMap<String, Vec<HashMap<String, HashMap<String, Value>>>>> = 

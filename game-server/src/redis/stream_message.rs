...
impl StreamMessage {
    ...
    pub fn to_json(&self) -> Result<String> {
        serde_json::to_string(&self.data).map_err(ServerError::Serialization)
    }
...
fn extract_string(map: &HashMap<String, Value>, key: &str) -> Result<String> {
    match map.get(key) {
        Some(Value::BulkString(bytes)) => String::from_utf8(bytes.clone())
            .map_err(|e| ServerError::Parse(format!("Invalid UTF-8 for {}: {}", key, e))),
        _ => Err(ServerError::Parse(format!(
...
mod tests {
    ...
    fn test_stream_message_from_redis() {
        ...
        map.insert(
            "from".to_string(),
            Value::Data(b"0x1234567890abcdef".to_vec()),
        );
        map.insert(
            "to".to_string(),
            Value::Data(b"0xfedcba0987654321".to_vec()),
        );

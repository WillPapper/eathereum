...
impl StablecoinMonitor {
    async fn new(rpc_url: String, tx_broadcaster: broadcast::Sender<TransactionData>) -> Result<Self> {
        // Create provider
    ...
    async fn check_new_blocks(&self) -> Result<()> {
        ...
        while last_processed < latest_block {
            ...
            if let Some(block) = self.provider.get_block_by_number(last_processed.into(), true).await? {
        ...
        for tx in block.transactions.into_transactions() {
            ...
            if let Some(to) = tx.to() {
                if let Some(stablecoin_info) = self.stablecoins.get(&to) {
                    ...
                    if let Some(tx_data) = self.parse_stablecoin_transaction(
                        &tx,
                        to,
                        stablecoin_info,
                        block_number,
